package cn.qpwa.common.easemob.comm;
...
public class ClientContext {
	...
	private void initFromPropertiesFile() {
		...
		try {
			InputStream inputStream = ClientContext.class.getClassLoader().getResourceAsStream("easemob-config.properties");
			p.load(inputStream);
		} catch (IOException e) {
			log.error(MessageTemplate.print(MessageTemplate.FILE_ACCESS_MSG, new String[]{"easemob-config.properties"}));
		...
		if( StringUtils.isBlank(protocal) || StringUtils.isBlank(host) || StringUtils.isBlank(org) || StringUtils.isBlank(app) || StringUtils.isBlank(clientId) || StringUtils.isBlank(clientSecret) || StringUtils.isBlank(impLib) ) {
			log.error(MessageTemplate.print(MessageTemplate.INVAILID_PROPERTIES_MSG, new String[]{"easemob-config.properties"}));
			return; // Context not initialized
		}
