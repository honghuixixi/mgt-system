			...
			if("GET".equals(method)){
						...
						if (fileItems != null) {
							...
							for (FileItem fileItem : fileItems)
							{
								if (fileItem.isFormField()) {
									if(jsonObject.containsKey(fileItem.getFieldName())){
									jsonObject.put(fileItem.getFieldName(), jsonObject.getString(fileItem.getFieldName())+","+fileItem.getString("utf-8"));
									}else{
				...
				if(contentType!=null&&contentType.toLowerCase().indexOf("application/json")!=-1){
					...
					if (StringUtils.isNotBlank(jsonData)) {
						throw new Exception("客户端提交的Json数据为空！");
					}
					if ((!jsonData.startsWith("{")) || (!jsonData.endsWith("}"))) {
						throw new Exception("客户端拼装的Json格式不对!");
					}
					try {
						jsonObject =JSONObject.fromObject(jsonData);
					} catch (JSONException e) {
						throw new Exception("客户端拼装的Json串格式不对!",e);				}
