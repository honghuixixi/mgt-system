...
<html>
	...
	<body>
		<form style="width:100%;padding:0;	" class="form-horizontal" id="form" action="${base}/order/sopPicking.jhtml">
...
<div class="page-content">
    ...
    <tbody>
	  ...
	  <tr height="60">
    	...
    	<input name="netPrice" type="hidden"  pkno="${item.PK_NO}" value="${item.NET_PRICE?string("0.00")}">
	    <td width="126" style="border-right:1px solid #000;"><span>${item.UOM_QTY}${item.UOM}</span></td> 
	    [#if item.QTY1 > 0]
...
<script type="text/javascript">
 	function checkForm(){
		...
		var price = document.getElementsByName("netPrice");
		 for ( var  i = 0 ;i < object.length;i ++ ){
			...
			if( !exp.test(price[i].value)){
				top.$.jBox.tip('请输入正确的价格');
				return false;
			}
			...
			if(i != object.length-1){
				str =str + object[i].getAttribute('pkno')+","+object[i].value+","+price[i].value+";";
			}else{
				str =str + object[i].getAttribute('pkno')+","+object[i].value+","+price[i].value;
			}
		 }
