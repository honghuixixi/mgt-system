		...
		<script  type="text/javascript">
			$(document).ready(function(){
					...
					mgt_util.jqGrid('#grid-table',{
						...
						url:'${base}/donation/donationTaskLogList.jhtml',
	 					colNames:['客户编码','客户名称','筛选规则','条件','类型','发放日期','操作'],
	 					width:1000,
					   	colModel:[
					   		...
					   		{name:'',width:150,formatter:function(value,row,index){
									var str= '&nbsp;&nbsp;&nbsp;<button type=button class="btn btn-info edit" onclick=detailUI("'+index.PK_NO+'","'+index.DONATION_TYPE+'")>详情</button>&nbsp;';
									if(index.STATUS_FLG=='S'){
										  str+='<button type=button class="btn btn-info edit"  onclick=send("'+index.PK_NO+'")>发放</button>';
									}
									if(index.STATUS_FLG=='A' && index.DONATION_TYPE=='S'){
										  str+='<button type=button class="btn btn-info edit"  onclick=cancle("'+index.PK_NO+'")>撤销</button>';
									}
									if(index.CP_STATUS_FLG!='N' && index.DONATION_TYPE=='C'){
										  str+='<button type=button class="btn btn-info edit"  onclick=cancle("'+index.PK_NO+'")>撤销</button>';
									}
									return str;
								}
			...
			<div style="border-bottom:1px solid #f39801;overflow:hidden;padding-bottom:10px;">
	            <form class="form form-inline queryForm"  id="query-form" name="form1">
					 ...
					 <div class="form-group">
		                 ...
		                 <input type="text" class="form-control" name="nameOrCustCode" id="nameOrCustCode" placeholder="输入客户名称/编码搜索" style="width:150px;" >
					 </div>
					 <div class="form-group">
					 	 <label class="control-label" style=" vertical-align: middle;">筛选规则</label>
					 	 <select class="form-control" id="filterRule" name="filterRule" style="width:120px;">
									<option value="">请选择</option>
	    							<option value="A">按订单金额</option>
	    							<option value="B">按订货频率</option>
	    							<option value="M">手工筛选</option>
						</select>
					 </div>
					 <div class="form-group">
					 	 <label class="control-label" style=" vertical-align: middle;">类型</label>
					 	 <select class="form-control" id="donationType" name="donationType" style="width:120px;">
									<option value="">请选择</option>
	    							<option value="S">实物</option>
						...
						<div class="search_cBox" style="bottom:20px;">
							 <div class="form-group">
						        <button type="button" class="btn_divBtn" id="stock_query" data-toggle="jBox-query"><i class="icon-search"></i> 搜 索 </button>
							 	<button class="search_cBox_btn btn btn-info"  onclick="backreturn();return false;">返回</button>
	           			    </div>
           			    </div>
	            </form>
	        </div>
