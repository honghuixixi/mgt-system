			...
			<div id="currentDataDiv" action="menu" >
				<div class="control-group sub_status" style="position:relative;">
					<ul class="nav nav-pills" role="tablist">
						...
						<li role="presentation" id="B" class="sub_status_but"><a> 待发出退货单</a></li>						
						<li role="presentation" id="E" class="sub_status_but"><a> 已发出退货单</a></li>
...
<script type="text/javascript">
	$().ready(function() {
	    ...
	    $("li.sub_status_but").on("click",function(){
	    	...
	    	if(idVal=='A'){
	    		$('#grid-table').GridUnload();//重绘
	    		var postData={orderby:"MAS_NO",statusFlg:"D",masCode:'WHRMERGE',type:'A'};
				mgt_util.jqGrid('#grid-table',{
					postData: postData,
					url:'${base}/returnmergeorder/returnmergeorderlist.jhtml',
 					colNames:['','日期','汇总单号','退货仓库编码','退货仓库','物流商','状态','操作'],
	    		...
	    		$('#grid-table').GridUnload();//重绘
	    		var postData={orderby:"MAS_NO",statusFlg:"R",masCode:'WHRMERGE,VNRMERGE',type:'B'};
				mgt_util.jqGrid('#grid-table',{
					postData: postData,
					url:'${base}/returnmergeorder/returnmergeorderlist.jhtml',
 					colNames:['','日期','汇总单号','单据类型','收货方编码','收货方名称','收货方商户','状态','操作'],
 					rowNum:1000,
 					rowList:[],
				   	colModel:[
				   		{name:'PK_NO',index:'PK_NO',width:0,hidden:true,key:true},
				   		...
				   		{name:'ACC_CODE',align:"center",width:100},
				   		{name:'ACC_NAME',align:"center",width:100},
				   		{name:'ACC_NAME',align:"center",width:100},
				   		{name:'STATUS_FLG',width:80,align:"center",editable:true,formatter:function(data){
							if(data=='D'){
								return '已发货';
					...
					var ids=jQuery("#grid-table").jqGrid('getDataIDs'); 
						for(var i=0; i<ids.length; i++){ 
							...
							detail ="<button type='button' class='btn btn-info edit' id='"+id+"' data-toggle='jBox-show' href='${base}/returnmergeorder/returnmergeorderdetailUI.jhtml'>明细 </button>";
							detail =detail +"<button type='button' class='btn btn-info edit' id='"+id+"' data-toggle='jBox-show' href='${base}/returnmergeorder/whdeliverUI.jhtml'>发货</button>";
							jQuery("#grid-table").jqGrid('setRowData', ids[i], { detail: detail }); 
						};
						//table数据高度计算
						cache=$(".ui-jqgrid-bdivFind").height();
	    		...
	    		$('#grid-table').GridUnload();//重绘
	    		var postData={orderby:"MAS_NO",statusFlg:"D",masCode:'WHRMERGE,VNRMERGE',type:'B'};
				mgt_util.jqGrid('#grid-table',{
					postData: postData,
					url:'${base}/returnmergeorder/returnmergeorderlist.jhtml',
 					colNames:['','日期','汇总单号','单据类型','收货方编码','收货方名称','收货方商户','状态','操作'],
					...
					var ids=jQuery("#grid-table").jqGrid('getDataIDs'); 
						for(var i=0; i<ids.length; i++){ 
							...
							detail ="<button type='button' class='btn btn-info edit' id='"+id+"' data-toggle='jBox-show' href='${base}/returnmergeorder/returnmergeorderdetailUI.jhtml'>明细 </button>";
							detail =detail +"<button type='button' class='btn btn-info edit' id='"+id+"' data-toggle='jBox-show' href='${base}/returnmergeorder/whdeliverUI.jhtml'>修改</button>";
							jQuery("#grid-table").jqGrid('setRowData', ids[i], { detail: detail }); 
						};
						//table数据高度计算
						cache=$(".ui-jqgrid-bdivFind").height();
	    		...
	    		$('#grid-table').GridUnload();//重绘
	    		var postData={orderby:"MAS_NO",statusFlg:"P",masCode:'WHMERGE,VNMERGE',type:'C'};
				mgt_util.jqGrid('#grid-table',{
					postData: postData,
					url:'${base}/returnmergeorder/returnmergeorderlist.jhtml',
					...
					var ids=jQuery("#grid-table").jqGrid('getDataIDs'); 
						for(var i=0; i<ids.length; i++){ 
							...
							if(rowData.ACC_CODE == '${currentWhc}'){
							...
							}else if(rowData.DEST_WH_C == '${currentWhc}'){
								detail ="<button type='button' class='btn btn-info edit' id='"+id+"' data-toggle='jBox-show' href='${base}/returnmergeorder/returnmergeorderinfoUI/N.jhtml'>退货</button>";
							}
							jQuery("#grid-table").jqGrid('setRowData', ids[i], { detail: detail }); 
						};
						//table数据高度计算
				...
				mgt_util.jqGrid('#grid-table',{
					...
					url:'${base}/returnmergeorder/returnmergeorderlist.jhtml',
 					...
 					rowList:[],
				   	colModel:[
				   		{name:'PK_NO',index:'PK_NO',width:0,hidden:true,key:true},
				   		{name:'MAS_DATE1',align:"center",width:140},
				   		{name:'MAS_NO',align:"center",width:140},
				   		{name:'MAS_CODE',align:"center",width:140,editable:true,formatter:function(data){
							if(data=='WHRMERGE'){
								return '仓库退货单';
							}else{
								return '供应商退货单';
	   					...
	   					{name:'DEST_WH_C',align:"center",width:140},
	   					{name:'WH_NAME',align:"center",width:140},
				   		{name:'ACC_CODE',align:"center",width:140},
				   		{name:'ACC_NAME',align:"center",width:140},
				   		{name:'STATUS_FLG',width:80,align:"center",editable:true,formatter:function(data){
							if(data=='D'){
								return '已发货';
							}else if(data=='P'){
