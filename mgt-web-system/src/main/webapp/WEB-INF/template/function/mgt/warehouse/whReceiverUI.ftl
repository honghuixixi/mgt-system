			...
			$(document).ready(function(){
				...
				mgt_util.jqGrid('#grid-table',{
					...
					url:'${base}/warehouse/itemListQuery.jhtml',
 					colNames:['商品编码','条码','商品名称','单位','请求数量','实际发货量','收货量'],
 					multiselect:false,
 					rowNum:1000,
				   	colModel:[	 
						...
						{name:'PLU_C',width:60},
				   		...
				   		{name:'QTY1', width:40,formatter:function(data,row,rowObject){
								//默认单据商品数量
						   		stkQty = rowObject.QTY1;
						   		if (!stkQty && typeof(stkQty)!="undefined" && stkQty!=0){  
						   			stkQty = '';
						   		}
						   		if("WHVNDIN"=='${masCode}'){
									return '<input type="text" class="deliverQty" stkQty="'+stkQty+'" name="qty" pkNo="'+rowObject.STK_C+'"  value="'+rowObject.QTY1+'">';
								}else{
									return '<input type="text" disabled="true" class="deliverQty" stkQty="'+stkQty+'" name="qty" pkNo="'+rowObject.STK_C+'"  value="'+rowObject.QTY1+'">';
								}
							}},
				   	],
				  });
			...
			function checkForm(){
				...
				var object = document.getElementsByName("qty");
				 for ( var  i = 0 ;i < object.length;i ++ ){
					...
					//数值非0开头
//					if(/[1-9][0-9]{0,5}/.test(object[i].value)){
