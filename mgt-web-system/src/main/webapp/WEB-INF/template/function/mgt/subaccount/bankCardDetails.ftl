		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				mgt_util.jqGrid('#grid-table',{
					url:'${base}/subaccount/bankCardPage.jhtml?id=${id}',
					rownumbers : false,
 					colNames:['','账号','行别','地区','开卡名称','行号','开户行','绑定时间','操作'],
				   	colModel:[
				   		{name:'ID',index:'ID',width:0,hidden:true,key:true},
				   		{name:'BANKCARDNO',width:120},
				   		{name:'BANKTYPENAME',width:120},
				   		{name:'BANKADDRNO', width:120},
				   		{name:'BANKCARDNAME', width:120},
				   		{name:'BANK_CODE',width:120,align:"center",editable:true},
				   		{name:'BANKTYPENAME',width:120,align:"center",editable:true},
						{name:'CARDBINDTIME', width:120, formatter:function(data){
							if(data==null){return '';}
							var date=new Date(data);
							return date.format('yyyy-MM-dd');
							}},
						{name:'', width:120,formatter:function(value,row,index){
							
							if(index.SUBACCOUNT_TYPE=='9001'&&deleteFlag=='1'){
