   ...
   <script  type="text/javascript">
			$(document).ready(function(){
				...
				mgt_util.jqGrid('#grid-table1',{
					...
					multiselect:false,
 					colNames:['商品编码','条码','商品名称','单位','数量','操作'],
 					rowNum:1000,
				   	colModel:[	 
						{name:'STK_C',width:50},
						{name:'PLU_C',width:60},
			...
			$("#addMas").click(function(){
				...
				var postData={pkNo:${pkNo},masCode:"${masCode}",vendorCode:"${vendorCode}",keyWord:$("#keyWord").val()};
				mgt_util.jqGrid('#grid-table',{
					postData: postData,
					url:'${base}/warehouse/allowStkMas.jhtml',
 					colNames:['','商品编码','条码','商品名称','单位','备货量','操作'],
 					rowNum:1000,
				   	colModel:[
						...
						{name:'PLU_C',width:60},
				   		...
				   		{name:'', width:40,align:"center",formatter:function(data,row,rowObject){
							return '<button type=button class="btn btn-info edit" onclick=addWhIo("'+rowObject.STK_C+'")>添加</button>';
							}
						} 							
				   	],
				  });
