...
$(function(){
	...
	var $tag=$("li.li_tag");
	$tag.click(function(){
		$(this).addClass("recom-on").siblings().removeClass("recom-on");
		});
	$(".tip1").click(function(){
		$(".show").show();
		$(".hide").hide();
    ...
    $(".Tab_body").hide().eq($('.Tab_list li').index(this)).show();}); 
    //tab切换
	})
//返回顶部
$(function(){
...
$(document).scroll(function(){
	...
	if(src>=40){
		$(".order-state").addClass("fiexd");
		}else{
		$(".order-state").removeClass("fiexd")	
		}
	})
...
var messages = {
	"shop.message.sysError":"服务器繁忙，请刷新重试",
	"shop.order.addressInfoRequired":"请完善收货人地址信息内容",
	"shop.order.settlementInfoRequired":"请完善支付信息内容",
	"shop.order.productRequired":"您还未选购商品",
	"shop.order.cancelOrder": "你确定要取消此订单？",
	"shop.message.success": "操作成功",
	"shop.message.error": "操作错误",
	"shop.message.defalutAddress": "此地址已经是默认地址了",
	"shop.dialog.ok": "确&nbsp;&nbsp;定",
	"shop.dialog.cancel": "取&nbsp;&nbsp;消",
	"shop.dialog.selectDelete": "请选择要删除的商品",
	"shop.dialog.cancelAttentComfirm": "您确定要取消关注吗？",
...
// 货币格式化不含金钱符号
function currency_0(value, showSign, showUnit) {
	if (value != null) {
		var price;
		if (setting.priceRoundType == "roundHalfUp") {
			price = (Math.round(value * Math.pow(10, setting.priceScale)) / Math.pow(10, setting.priceScale)).toFixed(setting.priceScale);
...
if ($.validator != null) {
	...
	$.validator.setDefaults({
		...
		errorPlacement: function(error, element) {
			var fieldSet = element.parent().next();
			if (fieldSet.size() > 0) {
