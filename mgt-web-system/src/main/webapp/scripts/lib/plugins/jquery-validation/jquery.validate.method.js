...
jQuery.validator.addMethod("abcnb",function(value, element) {
	return this.optional(element) || /^[a-zA-Z0-9]{8,14}$/.test(value);
},"8-14位字母或数字开头，允许字母数字");
...
jQuery.validator.addMethod("isArea", function(value, element) {
    if($("#areaId").val() == ''  || $($(".area_list").children("select")[2]).val() == ''|| $($(".area_list").children("select")[1]).val() == ''){
        return false;
    }
        return true; 
}, "请输入区域");
...
//1到100整数验证
jQuery.validator.addMethod("discNum",function(value, element) {
	 var tel = /^([0-9]\d{0,1}|100)$/;
	    return this.optional(element) || (tel.test(value));
},"请输入0到100整数")
