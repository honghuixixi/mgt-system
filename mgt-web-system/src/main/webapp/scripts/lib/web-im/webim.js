...
var showChatUI = function () {
	...
	login_userEle.innerHTML = $("#currentUserName").val();
	login_userEle.setAttribute("title", $("#currentUserName").val());
};
//登录之前不显示web对话框
		...
		success : function(roster) {
			...
			if (bothRoster.length > 0) {
				...
				for(var i=0;i<bothRoster.length;i++){
					usernames.push(bothRoster[i].name);
				}
				$.ajax({
					url:$("#basepath").val()+'/easeMob/getIUserNickName.jhtml',
					sync:false,
					type : 'post',
					dataType : "json",
					...
					success : function(data) {
						var nicknames = eval(data);
						for(var i=0;i<bothRoster.length;i++){
							bothRoster[i].nickname = nicknames[bothRoster[i].name];
							bothRoster[i].headimg  = nicknames[bothRoster[i].name + "_head_img"];
						}
						
						curroster = bothRoster[0];
						buildContactDiv("contractlist", bothRoster);//联系人列表页面处理
						//var chatTo = $("#chatTo").val();
						var chatTo = nicknames[$("#chatTo").val()];
						if(chatTo){
							for(var k in bothRoster){
								if(bothRoster[k].name == chatTo.toLocaleLowerCase()){
									setCurrentContact(bothRoster[k].name);
								}
							}
						}else{
							if (curroster){
	...
	for (var i = 0; i < rosterMsg.length; i++) {
		...
		if (contact.subscription == 'both') {
			...
			if (!isexist) {
				...
				$('<img>').attr({
					"src" : "../styles/css/web-im/img/head/contact_normal.png"
				}).appendTo(lielem);
...
var buildContactDiv = function(contactlistDivId, roster) {
	...
	for (i = 0; i < roster.length; i++) {
		...
		var lielem = $('<li>').attr({
			...
			'className' : 'offline',
			'type' : 'chat',
			'displayName' : roster[i].nickname
		}).click(function() {
			chooseContactDivClick(this);
		});
		//$('<img>').attr("src", "../styles/css/web-im/img/head/contact_normal.png").appendTo(lielem);
		//$('<img>').attr("src", "../styles/css/web-im/img/listCart-img.jpg").appendTo(lielem);
		if(roster[i].headimg==null || roster[i].headimg == ""){
			$('<img>').attr("src", "../styles/css/web-im/img/head/contact_normal.png").appendTo(lielem);
		}else{
			$('<img>').attr("src", roster[i].headimg).appendTo(lielem);
		}
		$('<span>').html(roster[i].nickname).appendTo(lielem);
		$('#contactlistUL').append(lielem);
	}
...
var showContactChatDiv = function(chatUserId) {
	...
	if (chatUserId.indexOf(groupFlagMark) >= 0) {
		...
		$.ajax({
			...
			success : function(data) {
				var nicknames = eval(data);
				dispalyTitle = "与" + nicknames[chatUserId] + "聊天中";
			}
		});
...
var createMomogrouplistUL = function createMomogrouplistUL(who, message) {
	...
	imgelem.setAttribute("src", "../styles/css/web-im/img/head/contact_normal.png");
	//imgelem.setAttribute("src", "../styles/css/web-im/img/listCart-img.jpg");
...
var appendMsg = function(who, contact, message) {
	...
	$.ajax({
		...
		success : function(data) {
			var nicknames = eval(data);
			headstr = [ "<p1>" + nicknames[who] + "   <span></span>" + "   </p1>",
			    			"<p2>" + getLoacalTimeString() + "<b></b><br/></p2>" ];
		}
	});
...
Easemob.im.EMOTIONS = {
    path: '../styles/css/web-im/img/faces/'
    , map: {
        '[):]': 'ee_1.png',
        '[:D]': 'ee_2.png',
        '[;)]': 'ee_3.png',
